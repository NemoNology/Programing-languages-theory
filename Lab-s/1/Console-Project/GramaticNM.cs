namespace Console_Project
{
    public class GramaticNM
    {
        ...
        /// <summary>
        /// ѕереводит строку на формальный €зык
        /// </summary>
        /// <param name="text">—трока дл€ перевода</param>
        /// <returns>—трока на формальном €зыке</returns>
        public string Translate(string text = "S", int maxRepetitionsCount = 1000, bool isTreePrinted = false)
        {
            ...
            while (count < maxRepetitionsCount)
            {
                ...
                count++;
                isEnd = true;
                // примен€ем по очереди каждое правило €зыка к строке
                foreach (var rule in Rules)
                {
                    string key = rule.Key;
