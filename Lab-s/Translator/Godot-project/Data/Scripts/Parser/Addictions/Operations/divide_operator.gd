class_name DivideOperator extends Operator
...
func try_calculate(out_operands_stack: Array[Operand], out_parsing_result: ParsingResult) -> bool:
	...
	if operand2.get_value(out_parsing_result) == 0:
		out_parsing_result.errors.append(Error.new(Error.DivisionByZeroTypeErrorText, position))
		return false
